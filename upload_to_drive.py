...
Target: folder ID 19N3NGQundvR-gxaQsWBu71it-uVARn7B / subfolder "Link-Summarizer_vX.Y.Z"
Rules:
...
def get_subfolder_name() -> str:
    return f"Link-Summarizer_v{get_version()}"
...
def ensure_subfolder_tree(service, rel_parts: list[str], root_id: str, folder_cache: dict) -> str:
    """
    Walk rel_parts (path components of the directory relative to project root),
...
def rename_existing_folder(service, new_name: str, parent_id: str) -> str | None:
    """Find existing Link-Summarizer_v* folder and rename it. Return folder ID."""
    query = (
        f"name contains 'Link-Summarizer_v' "
        f"and '{parent_id}' in parents "
        f"and mimeType='application/vnd.google-apps.folder' "
        f"and trashed=false"
    )
...
def main():
    ...
    print(f"Uploading Link Summarizer v{get_version()} to Google Drive")
    ...
    all_files = collect_extension_files()
    ...
    print(f"\nDone. ({len(all_files)} files uploaded)")
